...
define({
	...
	// can be used here
	loader: (typeof process === 'undefined' && location.search.indexOf('config=prf') > -1) ?
		{
			// if we are using the full path to prf, we assume we are running
			// in a sibling path configuration
			baseUrl: '../..'
		} : {},

	// Non-functional test suite(s) to run in each browser
	suites: [ (typeof process === 'undefined' && location.search.indexOf('config=prf') > -1) ?
		'prf/test/all' : 'test/all' ],
